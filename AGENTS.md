...
- Access credentials are SQLite `access_keys` rows (schema v27); plaintext leaves the process only via the session-protected `GET /dashboard/api/v3/connection`. Persistence details: `docs/maintainer/storage-migration.md`.
...
- When editing docs, paired guide pages are English-first with a `.zh-CN.md` twin per page under `docs/user/` and `docs/maintainer/`; keep paths and TOCs consistent across each EN/ZH pair. User-visible facts are governed by code and the user guides in `docs/user/` (index: `docs/USER.md` / `docs/USER.zh-CN.md`). Do not write pass-through matrices, capability tables, or long circuit-breaker essays back into the README. Do not describe GOAT/SCNet as live routes. Custom API is a live route under the trusted-admin boundary; do not re-describe it as Phase-1 dormant, non-loopback HTTPS-only, public-DNS/private-denylist, connect-time DNS pinning, Direct/Manual-only, no production caller, verify `501`, or enable-blocking.
