...
- Keep the Provider catalog static and sealed. Do not introduce dynamic Provider plugins, user scripts, or runtime adapter loading. A user-deployed local service may be managed only through an approved static external-integration V3 adapter; OCG does not host or supervise that process.
