...
- 面板视图（侧栏顺序）：Dashboard / Access Keys / Accounts / Providers / Applications / Logs / Settings。
...
- 持久化：SQLite（当前 schema v26）。v24 新增转发实际路由段字段，v25 新增供应商模型目录快照表，用于保存 Zen Free 最后一次成功刷新结果。v26 新增供应商合约范围表 `provider_contract_scopes` 与模型协议表 `provider_contract_model_protocols`。首次迁到 v23 前仍会在 `data.sqlite` 同目录写一份不覆盖的 `data.sqlite.pre-v23.<timestamp>.bak`；源库早于 v22 时还会保留 `data.sqlite.pre-v22.<timestamp>.bak`。GUI 数据目录为 Windows `%USERPROFILE%\.ocg-mgr` 或 macOS/Linux `~/.ocg-mgr`；CLI 默认 `~/.ocg-mgr-cli`。
...
- `crates/ocg-core/src/provider_contracts.rs`：供应商合约范围、协议开关与模型协议证据；改 Providers 页、协议探测或目录快照时从这里入手。
