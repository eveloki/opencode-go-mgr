...
mod tests {
    ...
    fn ollama_cloud_seed_is_locked_and_never_enters_model_protocols() {
        ...
        // The ollama provider key has no snapshot arm at all.
        assert!(snapshot_protocols("ollama", "deepseek-v4-flash").is_empty());
        assert!(snapshot_protocols("ollama", "gpt-oss:120b").is_empty());
    }
}
