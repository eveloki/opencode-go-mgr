...
use crate::provider::{
    ...
    KIMI_CN_CHAT_COMPLETIONS_PATH, KIMI_CN_MESSAGES_PATH, KIMI_CN_OFFERING_ID, KIMI_PROVIDER_ID,
    MINIMAX_CN_ANTHROPIC_BASE_URL, MINIMAX_CN_BASE_URL, MINIMAX_CN_CHAT_COMPLETIONS_PATH,
    MINIMAX_CN_MESSAGES_PATH, MINIMAX_CN_OFFERING_ID, MINIMAX_PROVIDER_ID, OPENCODE_PROVIDER_ID,
...
fn fixed_provider_plans_expose_documented_chat_and_messages_routes() {
    let config = AppConfig::default();
    for (provider_id, offering_id, chat_base, chat_path, messages_base, messages_path, model_id) in [
        (
            MINIMAX_PROVIDER_ID,
