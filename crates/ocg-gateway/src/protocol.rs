...
mod tests {
    ...
    #[test]
    fn production_protocol_source_stays_pure_json_conversion() {
        let production = production_source(include_str!("protocol.rs"));
        assert!(
            production.contains("pub fn convert_request_json"),
            "protocol.rs must export convert_request_json"
        );
        assert!(
            production.contains("pub fn convert_response_json"),
            "protocol.rs must export convert_response_json"
        );
        assert!(
            production.contains("empty_response_id"),
            "response conversion must take injected empty_response_id"
        );
        assert!(
            production.contains("created_at"),
            "response conversion must take injected created_at"
        );
        for needle in [
            "CoreState",
            "RequestPlan",
            ...
            "redact_known_secret",
            "model_protocol",
            "command_code_model_protocol",
            "is_known_model",
            "resolve_upstream_format",
            "probe_protocols",
            "Bytes",
        ] {
            assert!(
                !production.contains(needle),
                "production ocg-gateway protocol source must not name `{needle}`"
            );
        }
    }
