...
## Schema v34 — Ollama Cloud 用量状态

v34 创建 `ollama_cloud_usage_state` 表。每个已配置账号一行，包含：
...
行以 `account_id` 为主键并 `ON DELETE CASCADE` 引用 `accounts(id)`：删除账号即删除用量状态；
清除 Cookie 会删除该行并让能力回到未配置态。迁移只做增量：不改变任何既有表、
行或路由事实；且由于该步骤在 v27 之后执行，不会产生新的 pre-v3 快照。
