...
- 当前 schema 是 **v33**。迁移新增非空 `account_model_capabilities.upstream_model` 并从 `model_id` 回填，保留全部既有路由。之后 Custom 可写入不同的公开与上游名称；迁移不做后缀规范化，也不生成 Alias。详见 `storage-migration.zh-CN.md`。
