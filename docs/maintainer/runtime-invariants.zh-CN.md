...
- 账号迁移只存在于 V3：`POST /accounts/transfer/export|preview|import`，且 V1 只允许从回环面板执行；转发 scheme 请求头不会赋予权限。导出即使在回环模式也必须再次验证当前管理员凭据。面板只会收到版本 1 的 Argon2id（64 MiB、3 次迭代、单 lane）+ AES-256-GCM 固定 AAD 密文包，绝不会收到账号 Key 明文。密码学工作在进程级单飞限制下执行，且不占用设置或 SQLite 锁；全部响应都带 `Cache-Control: no-store`。预览与导入复用同一解密/校验流程；导入在 KDF 后重新检查 CAS，只跳过目标端 Plan/名称重复项，并通过一次 SQLite 事务写入账号及 Custom 配置/能力，最后只递增一次 revision。包内重复或任一非法行会拒绝整个文件。Zen 与节点/浏览器/运行状态不会迁移；能力来源在目标端重建为 `import`；Custom 导入后禁用并待验证，ready 托管账号保留 Key，未完成托管账号不会导出 Key，并以禁用状态重置到首个步骤。
