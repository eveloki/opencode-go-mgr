...
| 接入凭证 | SQLite `access_keys`（当前 schema v32，v27 引入），经 `crates/ocg-core/src/gateway_keys.rs` 与 `dashboard_v3/keys.rs`。主 Key id 为 `PRIMARY_KEY_ID`。历史 `sub_gateway_keys` 不是现行权威 |
