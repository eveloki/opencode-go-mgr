...
| 接入凭证 | SQLite `access_keys`（schema v31），经 `crates/ocg-core/src/gateway_keys.rs` 与 `dashboard_v3/keys.rs`。主 Key id 为 `PRIMARY_KEY_ID`。历史 `sub_gateway_keys` 不是现行权威 |
| SQLite 库版本 | `crates/ocg-core/src/db.rs`（`CURRENT_SCHEMA_VERSION = 31`）；升级/备份/回滚约定：[maintainer/storage-migration.zh-CN.md](maintainer/storage-migration.zh-CN.md) |
...
3. **贡献者** — 维护者指南 `layout` → `development` → `architecture` → `state-and-lifecycle` → `http-routes` → `conventions`；编码时以 `AGENTS.md` 为准（V3 crate 拆分、`/dashboard/api/v3`、schema v31 `access_keys`、托管向导、刷新额度、协议表、Key 命名）。未版本化 `/dashboard/api` REST 与 Tauri `invoke` 均不是当前面板路径。
