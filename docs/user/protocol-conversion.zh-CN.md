...
OCG Manager 在一个端口上讲五种客户端协议，再把每份请求翻译成上游 Plan 真正听得懂的格式。转换层没有好奇心：解析别名、检查账号资格、应用适配器上限与已保存的供应商合约、检查按模型/按协议的 effective 状态，然后才决定透传或转换。强制关闭或全局关闭的协议优先——哪怕模型说自己支持。
...
透传矩阵（测试账号实测，2026-08-14）。✓ = 客户端协议原样转发；空 = 转换到该模型推荐协议。权威来源：`crates/ocg-domain/src/protocol.rs` 的 `MODEL_PROTOCOLS`。
