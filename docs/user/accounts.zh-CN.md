...
| Ollama Cloud | `ollama` / `cloud` | 是（启用门控：路由、控制面与用量路径全部就绪前不可启用） | 固定源、仅 Chat Completions（`https://ollama.com`，Bearer）；公开无鉴权目录刷新；账号级可选网页会话 Cookie，用于手动抓取 `https://ollama.com/settings` 用量页；unpriced |
...
**导入**，先预览归并结果，再确认写入。不再要求管理员账号二次确认。迁移密码至少 12 个字符，
应与文件分开传递；OCG Manager 不会保存，也无法找回。迁移仍只允许从节点自己的回环面板执行；
转发的 scheme 请求头不会让远程面板获得迁移权限。

当前 V3 payload 会迁移可用普通账号及其稳定 ID、ready 账号 Key、Custom Endpoint/公开模型 → 上游 ID 映射与验证状态、
...
会直接跳过。旧版 V1 包仍可导入，并继续采用原先的 Plan/名称重复跳过规则；V2 也继续兼容导入。
外层加密 envelope 仍为 v1，变化的是其中 portable payload 的版本。
...
合格账号（enabled + ready + 非空 Key）只会把可路由的**公开名称**暴露在带鉴权的 `GET /v1/models` 上；仅有上游 ID 不构成第二份公开目录。Custom 公开名称会解析到其配对的精确上游 ID，且不得抢占已发布的内置 Alias。raw 身份冲突会从发布列表中排除，并以 `ambiguous_model_id` 失败，绝不调用上游。未声明名称仍为未知（`400`）。更改 Endpoint、Key、映射或协议仍保持账号启用。Endpoint 与上游协议可在创建后编辑；配置与完整映射集通过一次 CAS 事务替换。禁用声明协议时，模型不可路由；不允许固定优先级回退或覆盖去启用未声明协议。Custom 流量仍为未定价：日志记录 `cost_state=unknown`，不扣额度，也没有 Custom 供应商用量刷新。`MODEL_PROTOCOLS` 仍只属于 Go；Custom 将客户端协议转换到账号唯一上游协议。
