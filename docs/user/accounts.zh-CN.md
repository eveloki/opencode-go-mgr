...
当前 V2 包会迁移可用普通账号及其稳定 ID、ready 账号 Key、Custom Endpoint/模型声明与验证状态、
主接入 Key 和所有有效子 Key、可迁移的路由/代理设置、Zen Free 启用状态与目录，以及 Provider
目录、证据和协议覆盖。同一稳定 ID 按迁移包内的可迁移字段归并；Plan 或名称相同但 ID 不同的记录
可以并存。账号顺序以迁移包为先，目标端独有账号保持原相对顺序并接在后面；目标端独有接入 Key
和 Provider 范围也会保留。
...
会直接跳过。旧版 V1 包仍可导入，并继续采用原先的 Plan/名称重复跳过规则。
