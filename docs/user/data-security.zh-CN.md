...
- **无跨节点同步**：每个节点由自己的面板管理，OCG Manager 不会在节点间自动同步账号凭据。需要迁移时，可显式生成密码加密的 `.ocgbackup` 文件。导出前必须再次验证当前管理员凭据，账号 Key 使用 Argon2id 派生密钥并以 AES-256-GCM 加密。迁移密码不会保存，也无法找回；应与文件分开保管和传递。浏览器 Profile、接入凭据和其他节点设置不在迁移包内。
