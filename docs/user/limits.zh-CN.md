...
- Command Code GOAT 与 SCNet Token Plans 可以保存为禁用的 `pending` 草稿（`routable=false`）。连接验证返回 `501`。它们没有已上线的推理、用量、计价、验证运行时或生产路由；会在 **供应商** 页显示为不可路由范围，探测不能把它们提升为生产路由。Custom API 已在 [账号](accounts.zh-CN.md#账号) 的受信管理员边界下上线路由；不计价，也没有官方用量路径。Custom 的目录、协议与价格控制在 **供应商** 页以隔离的 `CustomEndpoint` 范围呈现。
