...
选择器列表来自受保护的 `GET /dashboard/api/v3/application-models`：当前可路由的 OpenCode Go 别名与当前价格快照求交。highspeed 变体继承基价行。空交集是 `[]`，不是错误。它 **不是** 带鉴权的 `GET /v1/models`：后者公布最早 OpenCode Go 静态表授权且当前可路由的 Alias，以及合格 Custom 声明 ID；保存的其他内置供应商目录只能加入这些 Alias，不能创建新的 Alias。两条路径都是本地读取，不在上游实时抓目录或挑账号，只返回当前可路由且协议有效启用的模型。目录刷新是 **供应商** 页上的显式动作；价格刷新后，这里的 Go 别名可能变化。每次返回应用页都会重新加载这份本地列表。模型选择和编辑过的代码片段按应用缓存在当前页面会话里，刷新即重置。**恢复默认** 重置当前应用的模型选择与片段草稿。
