...
test("Windows release smoke waits only for bounded installer processes", () => {
  ...
  assert.match(smoke, /function Set-V3AutoStart/);
  assert.match(smoke, /function Set-LegacyBootstrapAutoStart/);
  assert.match(smoke, /expectedRevision/);
  assert.match(smoke, /processGeneration/);
  assert.match(smoke, /\/dashboard\/api\/v3\/settings/);
  assert.equal((smoke.match(/\/dashboard\/api\/settings/g) ?? []).length, 1);
  const bootstrapStart = smoke.indexOf("if ($bootstrapOverwrite)");
  const legacyBootstrapCall = smoke.indexOf("Set-LegacyBootstrapAutoStart -Enabled $true");
  const overwriteUpdate = smoke.indexOf("candidate overwrite update");
  const candidateV3Write = smoke.indexOf("Set-V3AutoStart -Enabled $true", overwriteUpdate);
  assert.ok(bootstrapStart >= 0 && legacyBootstrapCall > bootstrapStart);
  assert.ok(overwriteUpdate > legacyBootstrapCall);
  assert.ok(candidateV3Write > overwriteUpdate);
